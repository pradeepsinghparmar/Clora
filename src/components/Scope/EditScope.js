...
const EditScope = () => {
  ...
  const EditscopeData = location.state?.EditscopeData || {};
  const EditscopeId = EditscopeData.id;
  ...
  const [formData, setFormData] = useState({
    project_id: String(EditscopeData.project_id) || "",
    ig_id: String(EditscopeData.ig_id) || "",
    profile_id: String(EditscopeData.profile_id) || "",
    resource_id: String(EditscopeData.resource_id) || "",
  });
  ...
  const scopeSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.post(
          `https://ifhirehealth.clindcast.com/api/updateprojectscope/${EditscopeId}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
