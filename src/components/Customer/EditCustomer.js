...
const EditCustomer = () => {
  ...
  const EditCustomerData = location.state?.EditCustomerData || {};
  const EditcustomerId = EditCustomerData.id;
  ...
  const [formData, setFormData] = useState({
    ...
    linkedin: EditCustomerData.linkedin || "",
    website: EditCustomerData.website || "",
    twitter: EditCustomerData.twitter || "",
    business_phone: EditCustomerData.business_phone || "",
    business_email: EditCustomerData.business_email || "",
    billing_street: EditCustomerData.billing_street || "",
    billing_city: EditCustomerData.billing_city || "",
    billing_state: EditCustomerData.billing_state || "",
    billing_code: EditCustomerData.billing_code || "",
    billing_country: EditCustomerData.billing_country || "",
    // shipping_street: EditCustomerData.shipping_street || "",
    // shipping_city: EditCustomerData.shipping_city || "",
    // shipping_state: EditCustomerData.shipping_state || "",
    // shipping_code: EditCustomerData.shipping_code || "",
    // shipping_country: EditCustomerData.shipping_country || "",
    logo: EditCustomerData.logo || null,
  });
  ...
  const customerSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.post(
          `https://ifhirehealth.clindcast.com/api/updatecustomer/${EditcustomerId}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            ...
            <div className="col-md-12">
              <div className="box mb-2 form-box">
                <div className="box-top-title fhir-title">
                  <span className="pd-overview-count">Edit Customer </span>
                </div>
